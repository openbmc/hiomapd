			 ...
			 uint32_t offset_bytes, uint32_t count_bytes)
{
	...
	if (low_mem.flash_offset < context->current->flash_offset) {
		...
		rc = context->flash.copy(context, low_mem.flash_offset,
	...
	if ((high_mem.flash_offset + high_mem.size) >
		...
		rc = context->flash.copy(context, high_mem.flash_offset,
	 ...
	 */
	rc = context->flash.erase(context, low_mem.flash_offset,
	...
	if (low_mem.mem) {
		...
		rc = context->flash.write(context, low_mem.flash_offset, low_mem.mem,
				 low_mem.size);
		if (rc < 0) {
			goto out;
		}
	}
	rc = context->flash.write(context, flash_offset,
	...
	if (high_mem.mem) {
		...
		rc = context->flash.write(context, high_mem.flash_offset, high_mem.mem,
				 high_mem.size);
		if (rc < 0) {
			goto out;
		}
		...
		rc = context->flash.write(context, high_mem.flash_offset,
...
{
	int rc;
	uint32_t flash_offset, count_bytes = count << context->flash.block_size_shift;
	uint32_t offset_bytes = offset << context->flash.block_size_shift;

	switch (type) {
	case WINDOW_ERASED: /* >= V2 ONLY -> block_size == erasesize */
		flash_offset = context->current->flash_offset + offset_bytes;
		rc = context->flash.erase(context, flash_offset, count_bytes);
		if (rc < 0) {
			MSG_ERR("Couldn't erase flash\n");
			return rc;
		}
	...
	case WINDOW_DIRTY:
		 ...
		 */
		if (log_2(context->flash.mtd_info.erasesize) !=
...
{
	if (offset + size > (cur->size >> context->flash.block_size_shift)) {
		...
		MSG_ERR("Requested offset: 0x%x size: 0x%x window size: 0x%x\n",
			...
			cur->size << context->flash.block_size_shift);
		return -EACCES;
	}
...
void window_reset(struct mbox_context *context, struct window_context *window)
{
	...
	if (window->dirty_bmap) { /* Might not have been allocated */
		window_set_bytemap(context, window, 0,
				   window->size >> context->flash.block_size_shift,
				   WINDOW_CLEAN);
	}
	window->age = 0;
}
 ...
 */
	...
	if (offset > context->flash.flash_size) {
		MSG_ERR("Tried to open read window past flash limit\n");
		return -EINVAL;
	} else if ((offset + cur->size) > context->flash.flash_size) {
		/*
		...
		if (context->version == API_VERSION_1) {
			cur->size = align_down(context->flash.flash_size - offset,
					       1 << context->flash.block_size_shift);
		} else {
			/*
			 * Allow requests to exceed the flash size, but limit
			 * the response to the size of the flash.
			 */
			cur->size = context->flash.flash_size - offset;
		}
	}

	/* Copy from flash into the window buffer */
	rc = context->flash.copy(context, offset, cur->mem, cur->size);
	if (rc < 0) {
	 ...
	 * it there
	 */
	cur->size = align_up(rc, (1ULL << context->flash.block_size_shift));
