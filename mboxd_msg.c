		   ...
		   bool write_back)
{
	...
	MSG_DBG("BMC Events clr to: 0x%.2x\n", context->bmc_events);
...
static uint8_t get_suggested_timeout(struct mbox_context *context)
{
	...
	/*
	 * Worst case scenario we have to write back to flash and then load
	 * from flash an entire window each of this size.
	 * Note: we align up to the nearest two seconds because the timeout
	 * value is given in 2 second granularity.
	 */
	ret = align_up(2 * max_size_mb * FLASH_ACCESS_MS_PER_MB, 2000) / 1000;
...
static inline uint16_t get_lpc_addr_shifted(struct mbox_context *context)
{
	...
	MSG_DBG("LPC addr of cur window: 0x%.8x\n", lpc_addr);
...
static int check_req_valid(struct mbox_context *context, union mbox_regs *req)
{
	...
	if (seq == context->prev_seq && cmd != MBOX_C_GET_MBOX_INFO) {
		MSG_ERR("Invalid sequence number: %d, prev: %d\n", seq,
			context->prev_seq);
