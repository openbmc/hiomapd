...
MemInfo MemoryMap::getMappedMemory(struct mbox_context* context,
                                   uint32_t offset)
{
    MemInfo info;
    for (const auto entry : mappedMemorySet)
    ...
    auto memInfo = loadFileIntoMemory(context, offset);
...
std::pair<uint32_t, MemInfo> MemoryMap::loadFileIntoMemory(
    struct mbox_context* context, uint32_t offset)
{
    MemInfo info;
