...
```
	...
	Command:
		...
		Notes:
			...
			the first byte of data requested by the host. A large
			window is of course preferred and should corrspond to
			the default size returned in the GET_MBOX_INFO command.

			The format of the CREATE_{READ,WRITE}_WINDOW commands
			are identical.

			Writes/Mark Dirty/Erases to a read window will have
			undefined effects.
