...
pnor_partition_table*  endianFixup(const pnor_partition_table& src)
{
    size_t size = src.data.size << partition::block::shift;
    pnor_partition_table* dst =
        static_cast<pnor_partition_table*>(std::calloc(1, size));
    if (!dst)
    {
        std::cerr << "Out of memory. File : "
                  << __FILE__ << " Line : " << __LINE__;
    ...
    for (auto i = 0; i < src.data.entry_count; ++i)
    {
        auto psrc = &src.partitions[i];
        auto pdst = &dst->partitions[i];
        strcpy(pdst->data.name, psrc->data.name);
        // Just to be safe
