...
int init_vpnor(struct mbox_context* context)
{
    if (context && !context->backend->vpnor)
    {
        ...
        strncpy(context->backend->paths.ro_loc, PARTITION_FILES_RO_LOC,
                PATH_MAX);
        context->backend->paths.ro_loc[PATH_MAX - 1] = '\0';
        strncpy(context->backend->paths.rw_loc, PARTITION_FILES_RW_LOC,
                PATH_MAX);
        context->backend->paths.rw_loc[PATH_MAX - 1] = '\0';
        strncpy(context->backend->paths.prsv_loc, PARTITION_FILES_PRSV_LOC,
                PATH_MAX);
        context->backend->paths.prsv_loc[PATH_MAX - 1] = '\0';
        strncpy(context->backend->paths.patch_loc, PARTITION_FILES_PATCH_LOC,
                PATH_MAX);
        context->backend->paths.prsv_loc[PATH_MAX - 1] = '\0';
...
int vpnor_copy_bootloader_partition(const struct mbox_context* context)
{
    ...
    try
    {
        ...
        local.backend->copy(&local, hbbOffset,
                            static_cast<uint8_t*>(context->mem) + hbbOffset,
                            hbbSize);
    }
...
{
    if (context && context->backend && context->backend->vpnor)
    {
