...
fs::path Request::getPartitionFilePath(int flags)
{
    ...
    auto dst = fs::path(ctx->backend->paths.patch_loc) / partition.data.name;
            ...
            (PARTITION_PRESERVED | PARTITION_READONLY))
    {
        case PARTITION_PRESERVED:
            dst = ctx->backend->paths.prsv_loc;
            break;

        case PARTITION_READONLY:
            dst = ctx->backend->paths.ro_loc;
            break;

        default:
            dst = ctx->backend->paths.rw_loc;
    }
    ...
    if (flags == O_RDONLY)
    {
        dst = fs::path(ctx->backend->paths.ro_loc) / partition.data.name;
        assert(fs::exists(dst));
        return dst;
    }

    assert(flags == O_RDWR);
    auto src = fs::path(ctx->backend->paths.ro_loc) / partition.data.name;
...
size_t Request::clamp(size_t len)
{
    ...
    size_t partSize = partition.data.size << ctx->backend->block_size_shift;
...
size_t Request::fulfil(const fs::path &path, int flags, void *buf, size_t len)
{
    ...
    else
    {
        ...
        ctx->backend->set_bytemap(ctx, base + offset, len, FLASH_DIRTY);
    }
    munmap(map, fileSize);
    close(fd);
