...
Table::Table(const struct mbox_context* ctx) :
    ...
    blockSize(1 << ctx->flash.erase_size_shift), pnorSize(ctx->flash.flash_size)
{
...
{
    const fs::path roDir = ctx->flash.paths.ro_loc;
    const fs::path patchDir = ctx->flash.paths.patch_loc;
