...
Writing register 13 will trigger an interrupt to the host to tell it that a
response is ready and waiting to be read which we don't want to happen until
the BMC has written all of the other registers.
...
Command:
	...
	Response:
		...
		Args 6: Timeout (encoded as below)
	Notes:
		...
		choose some reasonable value. The suggested timeout supplied in
		arg[6] is encoded as half the actual timeout value and must be
		reconstructed as follows:
		`suggested timeout = (arg[6] * 2) seconds`
