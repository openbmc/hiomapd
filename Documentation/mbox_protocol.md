...
defined as being able to be acknowledged by the host must be upon receipt.
...
need to write 0xFF individually. The BMC must ensure that if the host
...
negotiation. A BMC Reboot event implies a BMC Windows Reset event.
...
The BMC may at some points require access to the flash and must set the BMC
Flash Control Lost event when accessing the flash behind the daemons back. When
this event is set the host must assume that the contents of the active window
could be inconsistent with the contents of flash.
	...
	Notes:
		...
		hints to the daemon. Defined Values:
			0x01 - Short Lifetime:
				...
				the daemon could mark data contained in a
...
0x80 - BMC MBOX Daemon Ready: (V2)
	...
	not set.
	Note that this bit being set is not a guarantee that the daemon
